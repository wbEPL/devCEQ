...
# Market inceomd --------------------------------------------------------
ym <- rlnorm(n, meanlog = 3, sdlog = .5) *
  hh_size /
  (runif(n, 0.8, 1.2) * median(hh_size))
ym[sample(1:n, n * 0.05)] <- 0
ym[sample(1:n, n * 0.01)] <- max(ym) * 5


## Simlate benefits --------------------------------------------------------
sim_ben_share_value <- function(
  n,
  base_value,
  share_ben = 0.3,
  min_share = 0.1,
  ...
  ben_values[ben_idx] <- base_value[ben_idx] *
    runif(length(ben_idx), min_share, max_share)
  ben_values[if_zero] <- 0
  return(ben_values)
}

# Pension benefits:
ben_pen <- gen_var_from_var(
  ym,
  corr = 0.25,
  probs = c(.7, .6, .5, .5),
  share_eligile = 0.9,
...
ben_unemp <- gen_var_from_var(
  ym,
  corr = -0.015,
  probs = c(.8, .5, .1, .075, .025),
  share_eligile = 0.4,
  min_share = 0.2,
  max_share = 0.5
)
...
ink_education <- sim_ben_share_value(n, hh_size, 0.80, 0.5, 0.8, hh_size > 2) *
  mean(yg) /
  15
