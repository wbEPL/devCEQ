...
all_decile_by <- dta_1_incid |>
  select(any_of(f_get_colname("decile_var"))) |>
  pull(1) |>
  unique() |>
  as.character()
decile_fltr <- all_decile_by[1]

all_groups <- dta_1_incid |>
  select(any_of(f_get_colname("group_var"))) |>
  pull(1) |>
  unique() |>
  as.character()
grpoup_fltr <- all_groups[2]

all_vars <- dta_1_incid |>
  select(any_of(f_get_colname("var"))) |>
  pull(1) |>
  unique() |>
  as.character()
var_fltr <- all_vars[1]
