...
Most microsimulation analyses are developed in Stata. They have several key components that make them efficient, transparent, and reproducible. Such components in the order of their utilization are:
...
5.  **Simulation results**. A single data file with all variables and income concepts was simulated during the analysis. Based on it, we compute poverty and inequality measures as well as the distributional effect of policies.
...
2.  Set global variable `path` in lines 10-30 to `/Example_FiscalSim/` and save it.
...
1.  Open `/02.Dofile/00_master.do` with previous changes and make sure that pre-simulation step was executed correctly.
...
4.  Excel file with the simulation results will open in the end and you will be able explore the simulation results.
